...
import { pokemonApi, useGetAllPokemonsQuery, useGetSinglePokemonQuery } from '../../api/api';
...
export default function Layout() {
  ...
  const { data: allData, isLoading: allLoading, isFetching: allFetching, error: allError } = useGetAllPokemonsQuery(
    { offset: (currentPage - 1) * 20, limit: 20 },
    { skip: isSearching }
  );
  const { data: singleData, isLoading: singleLoading, isFetching: singleFetching, error: singleError } = useGetSinglePokemonQuery(
    searchTerm.trim(),
    { skip: !isSearching }
  );
  ...
  const data = isSearching
    ...
    : allData;
  const isLoading = isSearching ? (singleLoading || singleFetching) : (allLoading || allFetching);
